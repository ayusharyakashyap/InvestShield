...
class ContentAnalysisService:
    ...
    def analyze_text(self, text: str, source: str = "manual") -> Dict[str, Any]:
        ...
        if self.ml_model and 'ensemble_model' in self.ml_model:
            return self._analyze_with_enhanced_model(text, text_lower, source)
        else:
            # Fallback to keyword-based analysis
            return self._analyze_with_keywords(text, text_lower, source)
    
    def _analyze_with_enhanced_model(self, text: str, text_lower: str, source: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Combine ML prediction with keyword analysis
            final_risk_score = fraud_probability
            is_suspicious = bool(ml_prediction == 1 or fraud_probability > 60)
            ...
            if fraud_probability > 80:
                explanation = f"High fraud risk detected by ML model ({fraud_probability:.1f}% confidence). Found {sum(len(kw) for kw in found_keywords.values())} fraud-related keywords."
            elif fraud_probability > 60:
                explanation = f"Medium fraud risk detected by ML model ({fraud_probability:.1f}% confidence). Exercise caution."
            elif fraud_probability > 40:
                explanation = f"Some suspicious patterns detected ({fraud_probability:.1f}% risk). Verify independently."
            else:
                explanation = "Content appears legitimate with no significant fraud indicators detected."
            
            # Generate recommendations with error handling
            try:
                recommendations = self._generate_enhanced_recommendations(fraud_probability, found_keywords)
            ...
            return self._create_analysis_response(
                text, final_risk_score, fraud_probability, fraud_type, 
    ...
    def _analyze_with_keywords(self, text: str, text_lower: str, source: str) -> Dict[str, Any]:
        ...
        base_risk_score = min(keyword_risk_score + pattern_risk_score - legitimate_score, 100)
